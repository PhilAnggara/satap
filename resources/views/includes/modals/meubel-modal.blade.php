...
<div class="modal fade text-left" id="tambah" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1"
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="{{ route('meubel.store') }}" method="post" enctype="multipart/form-data">
        @csrf
...
<div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
  <div class="modal-content">
    <div class="modal-header bg-dark white">
      <span class="modal-title" id="myModalLabel1">{{ $item->nama_barang }}</span>
      <button type="button" class="close rounded-pill" data-bs-dismiss="modal" aria-label="Close">
        <i data-feather="x"></i>
      </button>
    </div>
...
<div class="modal-dialog modal-dialog-scrollable modal-xl" role="document">
  <div class="modal-content">
    <div class="modal-header">
      <h5 class="modal-title" id="myModalLabel1">{{ $item->nama_barang }}</h5>
      <button type="button" class="close rounded-pill" data-bs-dismiss="modal" aria-label="Close">
        <i data-feather="x"></i>
      </button>
    </div>
    <div class="modal-body">
      ...
      @if ($item->gambar)
        ...
        <div class="d-flex justify-content-center">
          <a href="#" id="hapusGbr" class="hapus-gbr btn btn-sm icon btn-light" data-id="{{ $item->id }}" data-title="{{ $item->nama_barang }}">
        ...
        <form action="{{ route('delete-image', $item->id) }}" id="hapus-gbr-{{ $item->id }}" method="POST">
          @csrf
          <input type="hidden" name="table" value="meubel">
        </form>
      @else
...
<div class="modal fade text-left" id="edit-{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1"
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="{{ route('meubel.update', $item->id) }}" method="post" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <div class="modal-body">
          ...
          <div class="form-group">
            <label for="nama_barang">Nama Barang</label>
            <input type="text" class="form-control" name="nama_barang" id="nama_barang" value="{{ $item->nama_barang }}" required>
          </div>
